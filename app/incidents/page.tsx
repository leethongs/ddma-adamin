              ...
              {filtered.map(inc => (
                <tr key={inc.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-4">
                    <p className="font-medium text-gray-800">{inc.victim_name}</p>
                    <p className="text-xs text-gray-400">{inc.contact_number}</p>
                    {inc.aadhaar_number && <p className="text-xs text-gray-300">Aadhaar: {inc.aadhaar_number}</p>}
                  </td>
                  ...
                  <td className="px-5 py-4 text-gray-500 text-xs max-w-[180px]">
                    {inc.location_address ?? (inc.latitude ? `${inc.latitude.toFixed(4)}°N, ${inc.longitude?.toFixed(4)}°E` : "—")}
                  </td>
                  ...
                  <td className="px-5 py-4">
                    <Link href={`/incidents/${inc.id}`} className="bg-orange-500 text-white text-xs px-3 py-1.5 rounded-lg hover:bg-orange-600 transition-colors font-medium">
                      View →
                    </Link>
                  </td>
                </tr>
